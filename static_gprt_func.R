...
initgprt <- function(y, offset, parms, wt) {
...
splitgprt <- function(y, wt, x, parms, continuous)
{
...
GPRT_method=list(eval = evalgprt, split = splitgprt, init = initgprt)
...
compute_bic_gprt <- function(tree, data) {
  loglik <- -sum(tree$frame[tree$frame$var == "<leaf>", "dev"])
  n_params <- sum(tree$frame$var == "<leaf>") * 2
  n_obs <- nrow(data)
